...
def failure_analysis(eval_results: list[EvalResult], bottom_n: int = 10) -> list[dict]:
    ...
    results_with_avg.sort(key=lambda x: x["avg"])

    failures = []
    for item in results_with_avg[:bottom_n]:
        worst = item["worst_metric"]
        diagnosis, suggested_fix = diagnostic_tree.get(worst, ("Unknown", "Review manually"))
        failures.append({
