...
from config import JUDGE_MODEL, HUMAN_LABELS_PATH, get_llm_client
...
'''
    ...
    if client is None:
        # Chưa có API key → trả kết quả trung tính (pass test, không gọi LLM)
    ...
    try:
        try:
            ...
            # Một số endpoint/model không hỗ trợ json_object mode → gọi thường
            resp = client.chat.completions.create(
                model=JUDGE_MODEL, messages=messages, temperature=0.0,
            )
        ...
        return {"winner": "tie", "reasoning": f"Judge error: {e}",
                "scores": {"A": 0.0, "B": 0.0}}

    winner = str(data.get("winner", "tie")).strip().lower()
    if winner not in {"a", "b", "tie"}:
        winner = "tie"
    winner = {"a": "A", "b": "B", "tie": "tie"}[winner]
    ...
    if winner != "tie" and not reasoning:
        reasoning = f"Answer {winner} tốt hơn theo 3 tiêu chí đánh giá."
...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    return JudgeResult(
        ...
        scores_pass2={"A": pass2_raw["scores"].get("B", 0.0),
...
def cohen_kappa(judge_labels: list[int], human_labels: list[int]) -> float:
    ...
    if p_e >= 1.0:
        # Cả hai rater đồng ý hoàn toàn về phân bố nhãn → κ = 1.0 nếu p_o = 1
    ...
    if total == 0:
        return {"total_judged": 0, "position_bias_rate": 0.0,
                ...
                "verbosity_details": {"a_wins_a_longer": 0, "b_wins_b_longer": 0,
                                      "total_decisive": 0},
                "interpretation": "Không có kết quả judge."}
